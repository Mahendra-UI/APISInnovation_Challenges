...
@Component({
  selector: 'app-datasecuritychallenge',
  templateUrl: './datasecuritychallenge.component.html',
  styleUrls: ['./datasecuritychallenge.component.css']
})
export class DatasecuritychallengeComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private router: Router,
    private toastr: ToastrService,
     private apiSer: ApiService, private spinner: NgxSpinnerService, private sanitizer: DomSanitizer) {
      ...
      this.router.events.subscribe((url:any) => {
        if(router.url == '/problemstatements' || router.url == '/eligibility'){
          this.clickme(router.url)
        }
      })
      }
    ...
    clickme(event:any){
      if(event == '/eligibility'){
        if(window.screenY > 2600){
          window.scroll(0, 0)
        }
        window.scroll(0, 2600)
      }
      else{
        if(window.screenY > 1150){
          window.scroll(0, 0)
        }
