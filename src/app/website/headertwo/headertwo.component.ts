...
export class HeadertwoComponent implements OnInit {
  ...
  onWindowScrollImage() {
    let element = document.querySelector('.navbar-brand') as HTMLElement;
    if (window.pageYOffset > element.clientHeight) {
  ...
  constructor(private formBuilder: FormBuilder,
    private toastr: ToastrService,
     private apiSer: ApiService, private spinner: NgxSpinnerService, private sanitizer: DomSanitizer) { }

     ngOnInit(): void {
      this.RegForm = this.formBuilder.group({
        // applicantName: ['', Validators.required],
        applicantName: ['', [Validators.required,  Validators.pattern(/^[0-9a-zA-Z\s]*$/)]],

        participateType: ['', Validators.required],
        ideationType: [''],
        deploymentType: [''],
        startupType: [''],
        nameOfOrganization: ['', Validators.required, Validators.pattern(/^[0-9a-zA-Z\s]*$/)],
        location: ['', [Validators.required, Validators.pattern(/^[.0-9a-zA-Z\s,-\/,(\/,)\/]*$/)]],
        state: ['', Validators.required],
        typeResearch: ['', [Validators.required, Validators.pattern(/^[0-9a-zA-Z\s]*$/)]],
        education: ['', [Validators.required,  Validators.pattern(/^[0-9a-zA-Z\s]*$/)]],
    ...
    ideationTypeCheck(event: any){
      ...
      if(value == '2'){
        ...
        this.govformcontrols['typeResearch'].setValidators([Validators.required, Validators.pattern(/^[0-9a-zA-Z\s]*$/)])
        this.govformcontrols['typeResearch'].updateValueAndValidity();
      }
    }
